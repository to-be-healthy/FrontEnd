...
export const StudentAlarmItem = ({ data, sender }: AlarmItemProps) => {
    ...
    mutate(notificationId, {
      onSuccess: () => {
        if (
          ...
          return router.push(`/student/community/${targetId}`);
        }
        ...
        if (
          ...
          return router.push(`/student/schedule?tab=myReservation`);
        }

        if (type === 'SCHEDULE-WRITE' || type === 'SCHEDULE-REPLY') {
          return router.push(`/student/log/${targetId}`);
        }
      },
    });
  };
